---
##Write a heading of some sort and info with background and links
...
First, aggregate aggregate all variables that can be aggregated by the mean:
```{r,eval=TRUE}
...
Create mode function from other code
```{r,eval=TRUE}
...
Region code currently skips from 25 to 51. Revise this number below:
...
Note to Emma: Look into what data you need for Kurtosis, what it represents, etc.
Kurtosis:
```{r,eval=TRUE}
...
Compute the correlation between the value of improvments and value of repairs:
```{r,eval=TRUE}
res <- cor(projectData[,c(2,3)],use='complete.obs')
round(res, 2)
```

These variables are weakly correlated. Below is a visual representation of the correlation between improvements and repairs:
```{r,eval=TRUE}
library(car)
...
Let's start by looking at the relationship between the value of improvements and MPesa use:
Note for Emma: Should this be a box plot???
```{r}
...
Let's also look at the relationship between the value of improvements and roof material
For Emma: Need to rename this, use actual names of what 1 & 4 represent
```{r}
projectData=projectData[complete.cases(projectData),] 
